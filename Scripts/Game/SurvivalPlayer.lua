...
local FoodMultipler = 1.0
local WaterMultipler = 1.0

local FoodCostPerStamina = 0.05
local WaterCostPerStamina = 0.1
local SprintStaminaCost = 0.7 / 40 -- Per tick while sprinting

local FoodLostPerSecond = 120 / 3.5 / 24 / 60
local WaterLostPerSecond = 120 / 2.5 / 24 / 60
...
function SurvivalPlayer.sv_restoreFood( self, food )
	if self.sv.saved.isConscious then
		...
		self.sv.saved.stats.food = self.sv.saved.stats.food + food * FoodMultipler
...
function SurvivalPlayer.sv_restoreWater( self, water )
	if self.sv.saved.isConscious then
		...
		self.sv.saved.stats.water = self.sv.saved.stats.water + water * WaterMultipler
